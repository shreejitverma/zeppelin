...
if sys.version_info < (3,0,0):
    print(
        f'{__file__} requires Python 3, while Python '
        + str(f'{sys.version[0]} was detected. Terminating. ')
    )
...
def merge_pr(pr_num, target_ref):
    ...
    commit_date = run_cmd(
        ['git', 'log', f'{pr_branch_name}', '-1', '--pretty=format:%ad']
    )
...
if merge_commits := [
    ...
    message = get_json(f"{GITHUB_API_BASE}/commits/{merge_hash}")["commit"][
        "message"
    ]
