...
class PythonCompletion:
  ...
  def getMethodCompletion(self, objName, methodName):
    execResult = locals()
    try:
      exec(f"objectDefList = dir({objName})", _zcUserQueryNameSpace, execResult)
    except:
      self.interpreter.logPythonOutput(f"Fail to run dir on {objName}")
      self.interpreter.logPythonOutput(traceback.format_exc())
      return None
    else:
      objectDefList = execResult['objectDefList']
      return [
          completion for completion in objectDefList
          if completion.startswith(methodName)
      ]
...
while True:
  ...
  try:
    ...
    try:
      global_hook = intp.getHook('post_exec_dev') if req.isCallHooks() else None
    except:
      global_hook = None

    try:
      user_hook = __zeppelin__.getHook('post_exec') if req.isCallHooks() else None
    except:
      user_hook = None
